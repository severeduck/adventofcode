...
for token in tokens:
    type, value, line_index, _ = token # for a better understanding check PEP 448
    
    if type == 'DIGIT' or type == 'NUMERIC_WORD':
        if first_digit == NOT_SET and value is not None:
            first_digit = int(value)
        last_digit = int(value)
    elif type == 'NEWLINE' or type == 'END':
        c = first_digit * 10 + last_digit
        calibration_values_sum += c
        # print(c)
...
# expected_value = 666
# assert calibration_values_sum == 666, f'unexpected value: {calibration_values_sum}, it should be {expected_value} instead.'
